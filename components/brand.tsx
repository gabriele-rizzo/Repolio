...
export function Brand({ label, href }: BrandProps) {
    return (
        <Link href={href ?? "/"} className={cn("flex flex-row gap-2", !label && !href && "p-2 hover:bg-muted")}>
            <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-primary scheme-dark dark:scheme-light text-sidebar-primary-foreground">
                <Image src={Logo} alt="logo" width={64} height={64} className="size-6" />
            </div>
