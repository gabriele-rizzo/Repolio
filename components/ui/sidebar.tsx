...
function SidebarGroupLabel({
    ...
    return useRender({
        ...
        props: mergeProps<"div">(
            {
                className: cn(
                    // Collapsed, the label is faded out and pulled up by its own height so the group
                    // closes over it — it stays in the flow only so that move can animate. It must
                    // also stop taking pointer events there: an invisible 32px band otherwise sits on
                    // top of the row above it and eats that row's clicks (this is what made
                    // "Report template" unclickable, the Accounts label being parked over it).
